  ...
  return (
    <nav className="w-full border-b border-border bg-[#F2F2F2] dark:bg-zinc-900 fixed top-0 left-0 z-50 transition-colors duration-300">
      ...
      <div className="hidden lg:flex w-[1920px] max-w-full mx-auto h-[60px] px-6 py-4 justify-between items-center text-[24px] font-medium tracking-wide">
        ...
        {/* Item 3: Journal */}
        <Link href="/journal" className="text-foreground hover:text-foreground/80 transition-colors">
          Journal
        </Link>
        ...
        {/* Item 5: Search */}
        <button className="text-foreground hover:text-foreground/80 transition-colors cursor-pointer focus:outline-none">
          Search
        </button>

        {/* Item 6: Profile / Log In */}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 cursor-pointer focus:outline-none">
                  ...
                  <span className="truncate max-w-[100px] text-foreground text-[24px] font-medium">{user?.fullName.split(" ").pop()}</span>
                </button>
              </DropdownMenuTrigger>
        ...
        {/* Item 7: Cart & Theme Toggle */}
        <div className="flex items-center gap-6">
          <Link href="/cart" className="text-foreground hover:text-foreground/80 transition-colors">
            Cart (0)
          </Link>
          <Button
            variant="ghost"
            ...
            className="rounded-full w-8 h-8 text-muted-foreground hover:text-foreground cursor-pointer focus-visible:ring-0 focus-visible:ring-offset-0"
          >
      ...
      <div className="flex lg:hidden w-full h-[60px] items-center justify-between px-4 bg-background transition-colors duration-300">
        {/* Left: Logo */}
        <Link href="/" className="flex items-center text-foreground hover:opacity-90 transition-opacity">
          <svg xmlns="http://www.w3.org/2000/svg" width="115" height="19" viewBox="0 0 173 28" fill="none" className="h-[18px] w-auto">
        ...
        <div className="flex items-center space-x-2">
          <Link href="/cart" className="p-2 text-foreground hover:text-foreground/80 relative">
              ...
              <div className="flex flex-col space-y-6 mt-8">
                <div className="border-b border-border pb-4">
                  {isLoading ? (
                    ...
                    <Link href="/login">
                      <Button className="w-full text-sm font-semibold">Log In</Button>
                    </Link>
                  )}
                </div>

                <nav className="flex flex-col space-y-4 text-base font-medium">
                  <Link href="/shop" className="hover:text-primary transition-colors py-1">Shop</Link>
                  <Link href="/journal" className="hover:text-primary transition-colors py-1">Journal</Link>
                  <Link href="/about" className="hover:text-primary transition-colors py-1">About</Link>
                  <button className="hover:text-primary transition-colors py-1 text-left flex items-center gap-2 cursor-pointer focus:outline-none">
                    <SearchIcon className="h-4 w-4" /> Search
                  </button>
                  {user && (
                    <>
                      <Link href={user.role === "admin" ? "/admin" : "/"} className="hover:text-primary transition-colors py-1 flex items-center">
                        <LayoutDashboard className="mr-2 h-4 w-4" /> Dashboard
                      </Link>
                      <Link href="/profile" className="hover:text-primary transition-colors py-1 flex items-center">
                        <User className="mr-2 h-4 w-4" /> Profile
                      </Link>
                      <Link href="/settings" className="hover:text-primary transition-colors py-1 flex items-center">
                        <Settings className="mr-2 h-4 w-4" /> Settings
                      </Link>
                      <button onClick={logout} className="text-destructive hover:text-destructive/80 transition-colors py-1 text-left flex items-center focus:outline-none">
