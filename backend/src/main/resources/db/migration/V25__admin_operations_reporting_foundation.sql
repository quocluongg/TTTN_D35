...
create table if not exists categories (
    id uuid primary key default gen_random_uuid(), name varchar(255) not null unique, slug varchar(300), description text,
    parent_id uuid references categories(id) on delete set null, created_at timestamptz not null default now()
);
create table if not exists products (
    id uuid primary key default gen_random_uuid(), name varchar(500) not null, description text, brand varchar(255), origin varchar(255),
    thumbnail varchar(1000), category_id uuid references categories(id) on delete set null, warranty_months integer,
    discount_percent numeric(5,2), slug varchar(550), source_url varchar(2000),
    created_at timestamptz not null default now(), updated_at timestamptz
);
create table if not exists product_variants (
    id uuid primary key default gen_random_uuid(), product_id uuid not null references products(id) on delete cascade, sku varchar(150),
...
create unique index if not exists idx_product_variant_sku_unique on product_variants(sku) where sku is not null;
...
create table if not exists inventory_movements (
    id uuid primary key default gen_random_uuid(), variant_id uuid not null references product_variants(id) on delete restrict,
